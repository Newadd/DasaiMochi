...
void readTouch() {
  ...
  if(state == HIGH && lastTouchState == LOW) {
    ...
    else if(tapCount >= 4) { setExpression(SLEEPY); tapCount = 0; }
  }
...
void updateEffects() {
  ...
  if(isBlinking) {
    ...
    if(t >= 1.0) { isBlinking = false; eyeLid = 1.0; }
    else { eyeLid = 1.0 - sin(t * PI); if(eyeLid < 0) eyeLid = 0; }
  } else eyeLid = 1.0;
...
void spawnParticles(int count) {
  for(int i=0; i<count && particleCount < 10; i++) {
    particles[particleCount].x = random(20, SCREEN_WIDTH-20);
    particles[particleCount].y = random(20, SCREEN_HEIGHT-20);
    particles[particleCount].life = random(20, 50);
    particleCount++;
  }
}
