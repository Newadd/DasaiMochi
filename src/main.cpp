// ==================================================
// DASAI MOCHI PRO - VERSI SEDERHANA TANPA FONT BESAR
// Untuk SSD1306 OLED (128x64, I2C)
// ==================================================
...
// Nonaktifkan brownout (hapus komentar jika perlu)
// #include "soc/soc.h"
...
void startConfigPortal() {
  ...
  display.println("DasaiMochi-Setup");
...
void drawWorldClock() {
  ...
  char iStr[9];
  sprintf(iStr, "%02d:%02d", itm->tm_hour, itm->tm_min);
  display.print(iStr);
  ...
  char sStr[9];
  sprintf(sStr, "%02d:%02d", stm->tm_hour, stm->tm_min);
  display.print(sStr);
...
void drawForecastPage() {
  if (fcast[0].temp == 0 && fcast[0].dayName == "") {
    display.setCursor(20, 30);
    display.print("Loading...");
    return;
  }
  ...
  for (int i = 0; i < 3; i++) {
    ...
    // Ikon mini
    display.drawBitmap(x, 20, getMiniIcon(fcast[i].iconType), 16, 16, SSD1306_WHITE);
    
    // Suhu
    display.setTextSize(1);
    display.setCursor(x + 2, 45);
    display.print(String(fcast[i].temp) + "C");
    
    // Garis pemisah
    if (i < 2) display.drawLine(x + 35, 0, x + 35, 64, SSD1306_WHITE);
  }
}
...
// ========== SETUP & LOOP ==========
void setup() {
  // Nonaktifkan brownout jika perlu (hapus komentar)
  ...
  int connectTry = 0;
  while (WiFi.status() != WL_CONNECTED && connectTry < 30) {
    delay(500);
    Serial.print(".");
    connectTry++;
  }
  ...
  if (WiFi.status() != WL_CONNECTED) {
    Serial.println("WiFi GAGAL! Masuk config mode...");
    startConfigPortal();
    return;
  }
  ...
  if (ntpTry >= 20) {
    Serial.println("NTP GAGAL! Time tidak sync");
  } else {
    Serial.println("NTP OK! Time: " + String(time(nullptr)));
  }
